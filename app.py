...
def load_custom_css() -> None:
    """Загружает кастомный CSS для оформления дашборда."""
    st.markdown("""
    <style>
        /* Основной фон */
        .stApp {
...
def generate_cashflow_data() -> pd.DataFrame:
    """
    Генерирует данные о денежных потоках за 8 лет.
    
    Returns:
        pd.DataFrame: DataFrame с колонками (year, investments, revenue, payouts, ocf, cumulative)
    """
    try:
        years: List[int] = list(range(1, 9))
        investments: List[int] = [-800_000, -600_000, -400_000, -100_000, 0, 0, 0, 0]
        revenue: List[int] = [0, 0, 50_000, 300_000, 900_000, 3_500_000, 3_800_000, 4_200_000]
        payouts: List[int] = [0, 0, 0, 0, 0, -3_200_000, -3_200_000, -3_200_000]
        
        ocf: List[int] = [inv + rev + pay for inv, rev, pay in zip(investments, revenue, payouts)]
        cumulative: List[int] = [sum(ocf[:i+1]) for i in range(len(ocf))]
        ...
        logger.info("✅ Cash flow data generated successfully")
        return df
        
    except Exception as e:
        logger.error(f"❌ Error generating cashflow data: {e}")
        raise
...
def generate_geo_data() -> pd.DataFrame:
    """
    Генерирует данные о географических проектах.
    
    Returns:
        pd.DataFrame: DataFrame с информацией о проектах (location, lat, lon, project, co2_tons, status, type)
    """
    try:
        df = pd.DataFrame({
        ...
        logger.error(f"❌ Error generating geo data: {e}")
        raise
...
def generate_kpi_data() -> Dict[str, any]:
    """
    Генерирует KPI данные.
    
    Returns:
        Dict: Словарь с ключевыми показателями
    """
    try:
        kpi = {
            'co2_absorbed_total': 11_850,
            'tokens_issued': 11_850,
        ...
        logger.error(f"❌ Error generating KPI data: {e}")
        raise
...
def create_cashflow_chart(df: pd.DataFrame) -> go.Figure:
    """
    Создаёт интерактивный график денежных потоков.
    
    Args:
        df (pd.DataFrame): DataFrame с данными о денежных потоках
    ...
    try:
        ...
        logger.info("✅ Cash flow chart created successfully")
        return fig
...
def create_geo_map(df: pd.DataFrame) -> go.Figure:
    """
    Создаёт географическую карту проектов.
    ...
    try:
        ...
        logger.info("✅ Geo map created successfully")
        return fig
        
    except Exception as e:
        logger.error(f"❌ Error creating geo map: {e}")
        raise
...
def render_kpi_card(
    ...
    """
    Генерирует HTML для KPI карточки.
    ...
    Args:
        ...
        color (str): Hex цвет значения
        trend (str, optional): Тренд (▲/▼). Defaults to None.
        
    Returns:
        str: HTML код карточки
    """
...
# =============================================================================
# 🔄 UI: РЕНДЕРИНГ KPI РЯДОВ
# =============================================================================
def render_kpi_row_1(kpi_data: Dict) -> None:
    """Рендерит первый ряд KPI карточек (4 основные метрики)."""
...
def render_kpi_row_2(kpi_data: Dict) -> None:
    """Рендерит второй ряд KPI карточек (дополнительные метрики)."""
...
def render_header(timestamp: str) -> None:
    """Рендерит главный заголовок дашборда."""
    st.markdown(f"""
    <div class="main-header">
        <h1>🌱 TA Terra Angel — Регенеративный дашборд</h1>
        <p>Обновлено: {timestamp} • 1 токен = 1 тонна CO₂ + 12.5% операционной прибыли</p>
    </div>
    """, unsafe_allow_html=True)


def render_footer() -> None:
    """Рендерит футер дашборда."""
    st.markdown("""
    <div class="footer">
...
def render_sidebar_info() -> None:
    """Рендерит информацию в боковой панели."""
    with st.sidebar:
...
def main() -> None:
    ...
    try:
        ...
        # Рендерим заголовок
        render_header(current_timestamp)
        ...
        render_kpi_row_1(kpi_data)
        render_kpi_row_2(kpi_data)
        ...
        st.error(f"⚠️ Произошла ошибка при загрузке дашборда: {e}")
